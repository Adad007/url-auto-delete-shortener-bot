...
BOT_TOKEN = environ.get('BOT_TOKEN', "7363706919:AAGYQbhFVRuQ4N_OsLqfqThxMvgCfYl1S1A")
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '1467547148').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1467547148').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1467547148')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://adithmohan40:N83mlJiu22KAJX98@radha.xlw67va.mongodb.net/?retryWrites=true&w=majority&appName=radha")
DATABASE_NAME = environ.get('DATABASE_NAME', "radha")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'Telegram_files')

# Others
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002160604217'))
